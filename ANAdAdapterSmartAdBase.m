...
- (NSString *)keywordsFromTargetingParameters:(ANTargetingParameters *)targetingParameters {
    // Setting the location manually on Smart Display SDK from the location already retrieved by AppNexus
    if (targetingParameters.location != nil) {
        ANLocation *location = targetingParameters.location;
        if (location) {
            [SASConfiguration sharedInstance].manualLocation = CLLocationCoordinate2DMake(location.latitude, location.longitude);
        }
    }
    
    // No keyword targeting on Smart Display SDK calls
    return @"";
}
